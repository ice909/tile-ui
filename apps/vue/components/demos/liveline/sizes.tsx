...
export default defineComponent({
	...
	setup() {
		...
		return () => {
			...
			return (
				<section class="ll-demo" data-theme={theme.value} style={{ marginBottom: 24 }} aria-label={`${'Size variants'} demo`}>
					...
					<div class="ll-controls" role="group" aria-label={`${'Size variants'} feed state`}>
						<span>State</span>
					...
					<div class="ll-controls" role="group" aria-label={`${'Size variants'} volatility`}>
					...
					<div class="ll-controls" role="group" aria-label={`${'Size variants'} cadence`}>
						<span>Tick rate</span>
					...
					<div class="ll-controls" role="group" aria-label={`${'Size variants'} features`}>
						<span>Features</span>
					...
					<div class="ll-controls" role="group" aria-label={`${'Size variants'} window style`}>
						<span>Window</span>
...

					{
						<div class="ll-sizes">
							{compactSizes.map(([width, height]) => (
								<div key={width} class="ll-size">
									<span>
										{width} x {height}
									</span>
									<div class="ll-panel" style={{ width: `${width}px`, height: `${height}px`, maxWidth: '100%' }}>
										<Liveline
											{...common}
											data={data}
											value={value}
											aria-label={`${width} by ${height} compact chart, ${state.value}`}
											badge={width >= 200}
											grid={grid.value && width >= 200}
											momentum={momentum.value && width >= 200}
											style={{ height: '100%' }}
										/>
									</div>
								</div>
