...
export default defineComponent({
	...
	setup() {
		...
		return () => {
			...
			return (
				<section class="ll-demo" data-theme={theme.value} style={{ marginBottom: 24 }} aria-label={`${'Crypto'} demo`}>
					...
					<div class="ll-controls" role="group" aria-label={`${'Crypto'} feed state`}>
						<span>State</span>
					...
					<div class="ll-controls" role="group" aria-label={`${'Crypto'} volatility`}>
					...
					<div class="ll-controls" role="group" aria-label={`${'Crypto'} cadence`}>
						<span>Tick rate</span>
					...
					<div class="ll-controls" role="group" aria-label={`${'Crypto'} features`}>
						<span>Features</span>
						...
						{
							<>
								<button type="button" aria-pressed={depth.value} onClick={() => setDepth(!depth.value)}>
									Orderbook
								</button>
								<button type="button" aria-pressed={degen.value} onClick={() => setDegen(!degen.value)}>
									Degen
								</button>
							</>
						}
					</div>
					<div class="ll-controls" role="group" aria-label={`${'Crypto'} window style`}>
						<span>Window</span>
...

					{
						<div class="ll-panel">
							<Liveline
								{...common}
								aria-label={`${'Crypto'}: ${state.value}, ${paused.value ? 'paused' : 'running'}, ${formatDemoPrice(value)}`}
								data={data}
								value={value}
								mode={'line'}
								windows={windows}
								...
								orderbook={depth.value && data.length ? demoOrderbook(value, feeds.value[0]?.ticks ?? 0) : undefined}
								referenceLine={data.length ? { value: 67432, label: 'Open' } : undefined}
								degen={degen.value ? { scale: 1, downMomentum: true } : false}
								style={{ height: '320px' }}
							/>
						</div>
					}
