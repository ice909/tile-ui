...
export default defineComponent({
	...
	setup() {
		...
		return () => {
			...
			const ohlc = aggregateDemoCandles(data, candleWidth.value);
			...
			return (
				<section class="ll-demo" data-theme={theme.value} style={{ marginBottom: 24 }} aria-label={`${'Candlesticks'} demo`}>
...

					{
						<div class="ll-controls" role="group" aria-label="Candle interval">
							...
							{[1, 2, 5, 10].map((seconds) => (
								<button key={seconds} type="button" aria-pressed={candleWidth.value === seconds} onClick={() => setCandleWidth(seconds)}>
									{seconds}s
								</button>
							))}
							<button type="button" aria-pressed={lineMode.value} onClick={() => setLineMode(!lineMode.value)}>
								{lineMode.value ? 'Show candles' : 'Morph to line'}
							</button>
						</div>
					}
					{
						<div class="ll-panel">
							<Liveline
								{...common}
								aria-label={`${'Candlesticks'}: ${state.value}, ${paused.value ? 'paused' : 'running'}, ${formatDemoPrice(value)}`}
								data={data}
								value={value}
								mode={'candle'}
