---
title: ToggleGroup
description: Grouped toggle buttons.
---

> Use ToggleGroup to group mutually exclusive or multiple selectable toggles.

## Registry install

```bash
pnpm dlx shadcn@latest add @tile-ui/toggle-group
```

## Package usage

```tsx
import { TToggleGroup } from '@tile-ui/vue';

<TToggleGroup type="single" defaultValue="a">
	<TToggleGroupItem value="a">A</TToggleGroupItem>
	<TToggleGroupItem value="b">B</TToggleGroupItem>
</TToggleGroup>
```

## Highlights

- Single and multiple modes
- Item variants and sizes

## Registry dependencies

| Item           | Purpose                            |
| -------------- | ---------------------------------- |
| `toggle-group` | Component source and module styles |
| `core`         | Framework-agnostic logic helpers   |
| `styles`       | Shared SCSS tokens and globals     |

## API reference

### ToggleGroup

| Prop   | Type                     | Default |
| ------ | ------------------------ | ------- |
| `type` | `'single' \| 'multiple'` | —       |

### ToggleGroupItem

| Prop      | Type            | Default |
| --------- | --------------- | ------- |
| `value`   | `string`        | —       |
| `variant` | `ToggleVariant` | —       |
| `size`    | `ToggleSize`    | —       |

## Related docs

- [Toggle](/docs/components/toggle)
- [ButtonGroup](/docs/components/button-group)
- [Tabs](/docs/components/tabs)
