...
```tsx
<TTextarea label="Summary" helperText="Keep it short and specific for reviewers." />
```

Textarea mirrors the Input field structure, so the two can be mixed in the same form without styling differences:

```tsx
<TTextarea label="Question" error="Please provide at least 20 characters before submitting." modelValue="Too short" />
```
