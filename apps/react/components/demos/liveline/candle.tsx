...
export default function LivelineDemo() {
	...
	return (
		<section className="ll-demo" data-theme={theme} style={{ marginBottom: 24 }} aria-label={`${'Candlesticks'} demo`}>
			...
			<div className="ll-controls" role="group" aria-label={`${'Candlesticks'} feed state`}>
				<span>State</span>
			...
			<div className="ll-controls" role="group" aria-label={`${'Candlesticks'} volatility`}>
			...
			<div className="ll-controls" role="group" aria-label={`${'Candlesticks'} cadence`}>
				<span>Tick rate</span>
			...
			<div className="ll-controls" role="group" aria-label={`${'Candlesticks'} features`}>
				<span>Features</span>
			...
			<div className="ll-controls" role="group" aria-label={`${'Candlesticks'} window style`}>
				<span>Window</span>
			...
			{
				<div className="ll-controls" role="group" aria-label="Candle interval">
					<span>Candles</span>
					{[1, 2, 5, 10].map((seconds) => (
						<button key={seconds} type="button" aria-pressed={candleWidth === seconds} onClick={() => setCandleWidth(seconds)}>
							{seconds}s
						</button>
					))}
			...
			<div className="ll-status">
				...
				<span>tick: {cadence}ms</span>
				{
					<>
						...
						<span>live OHLC: {ohlc.live ? [ohlc.live.open, ohlc.live.high, ohlc.live.low, ohlc.live.close].map((v) => v.toFixed(2)).join(' / ') : '--'}</span>
					</>
				}
			</div>
