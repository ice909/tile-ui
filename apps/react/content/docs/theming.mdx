---
title: Theming
description: Configure shadcn-compatible semantic colors, Tile extension tokens, dark mode, scoped themes, and portal containers in React.
---
...
Keep your current shadcn semantic variables and import only Tile's extension tokens once in the application entry or root layout:
...
```tsx
import '@tile-ui/styles/scss/tokens.scss';
import '@tile-ui/styles/scss/reset.scss';
```

## Non-shadcn default setup

When Tile should provide the initial semantic palette, import `theme.scss`. It includes the Tile extension tokens, so a separate `tokens.scss` import is not needed.
...
For the legacy all-in-one behavior, use `globals.scss` instead of both imports:
...
Add the selected class or attribute to `html` so body-level portals inherit the same theme:
...
For system mode, use your existing theme manager, such as `next-themes`, to resolve `system` and update the `html` class. CSS can also follow the operating system directly:
...
```tsx
<section className="marketing-theme">
	<Button>Start a campaign</Button>
</section>
```

This works for components rendered inside the section. Portaled overlays need one additional step because a portal rendered under `body` is no longer a CSS descendant of the scoped element.

## Scoped themes and portals

Dialog, popover, select, tooltip, dropdown, and other overlays portal to `document.body` by default. A theme placed on `html` or `body` works automatically. For a subtree-only theme, pass the themed element through `PortalProvider`, or pass the same `container` directly to an overlay content component when only one overlay needs it.

```tsx
'use client';

import { useState } from 'react';
import { Dialog, DialogContent, DialogTrigger, PortalProvider } from '@tile-ui/react';

export function BrandedArea() {
	const [container, setContainer] = useState<HTMLDivElement | null>(null);

	return (
		<div ref={setContainer} className="marketing-theme">
