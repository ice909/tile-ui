...
import { lerp } from '../../src/liveline/math/lerp';
import { computeRange } from '../../src/liveline/math/range';
import { detectMomentum } from '../../src/liveline/math/momentum';
import { interpolateAtTime } from '../../src/liveline/math/interpolate';
import { niceTimeInterval } from '../../src/liveline/math/intervals';
import type { LivelinePoint } from '../../src/liveline/types';

// -- lerp --

describe('lerp', () => {
	it('clamps boosted speeds without NaN or overshoot at fractional frame times', () => {
		for (const dt of [8, 16, 33, 50]) {
			expect(lerp(10, 20, 1.2, dt)).toBe(20);
			expect(lerp(10, 20, -0.2, dt)).toBe(10);
		}
		expect(lerp(10, 20, 1.2, 0)).toBe(10);
	});
...
describe('computeRange', () => {
	const pts = (values: number[]): LivelinePoint[] => values.map((v, i) => ({ time: i, value: v }));
